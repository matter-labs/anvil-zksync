...
fn to_error_object(error: impl Into<ZksyncError>) -> ErrorObjectOwned {
    let error: ZksyncError = error.into();
    ErrorObjectOwned::owned(error.get_identifier().encode() as i32, error.get_message(), Some(error))
}
