...
"#,
        ...
        if self.zkos_config.use_zkos {
            let bin_path_msg = match &self.zkos_config.zkos_bin_path {
                Some(path) => path.green().to_string(),
...
"#
    ...
    /// Get the ZKOS configuration
    pub fn get_zkos_config(&self) -> &ZKOSConfig {
        &self.zkos_config
    }

    /// Check if ZKOS is enabled
    pub fn is_zkos_enabled(&self) -> bool {
