    ...
    impl IExecutor::StoredBatchInfo {
        pub(crate) fn new(value: &ZkosCommitment, commitment: [u8; 32]) -> Self {
            Self::from((
                ...
                // For Boojum OS batches we'll store here full state commitment
                ...
                // For Boojum OS batches we'll store batch output hash here
...
///
/// Assumes system log verification and DA input verification are disabled.
pub fn zksync_os_commit_batches_shared_bridge_call(
    l2_chain_id: L2ChainId,
    ...
    IExecutor::commitBatchesSharedBridgeCall::new((
        alloy::primitives::U256::from(l2_chain_id.as_u64()),
        alloy::primitives::U256::from(last_committed_l1_batch.batch_number + 1),
        alloy::primitives::U256::from(last_committed_l1_batch.batch_number + 1),
        zksync_os_commit_calldata(last_committed_l1_batch, batch).into(),
    ))
}

/// `commitBatchesSharedBridge` expects the rest of calldata to be of very specific form. This
/// function makes sure last committed batch and new batch are encoded correctly (assumes post gateway).
fn zksync_os_commit_calldata(
    last_committed_l1_batch: &ZkosCommitment,
    batch: &ZkosCommitment,
) -> Vec<u8> {
    let last_committed_batch_output: BatchOutput =
        crate::commitment_generator::zkos_commitment_to_vm_batch_output(last_committed_l1_batch);
