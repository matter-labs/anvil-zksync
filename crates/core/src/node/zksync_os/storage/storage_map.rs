...
use crate::node::zksync_os::execution::metrics::STORAGE_VIEW_METRICS;
...
use crate::node::zksync_os::BLOCKS_TO_RETAIN;
use dashmap::DashMap;
...
impl StorageMap {
    ...
    fn compact(&self) {
        ...
        self.persistent_block_upper_bound
            .store(can_compact_until, Ordering::SeqCst);
        ...
        tracing::info!("Compacting {} unique keys", map.len(),);

        self.persistent_storage_map
            .compact_sync(can_compact_until, map);
...
impl ReadStorage for StorageMapView {
    ...
    fn read(&mut self, key: Bytes32) -> Option<Bytes32> {
        ...
        for bn in (self.base_block + 1..self.block).rev() {
            if let Some(diff) = self.diffs.get(&bn) {
                ...
                if let Some(value) = res {
                    STORAGE_VIEW_METRICS.storage_access_latency[&"diff"]
                        .observe(started_at.elapsed());
                    STORAGE_VIEW_METRICS.storage_access_diffs_scanned[&"diff"]
        ...
        STORAGE_VIEW_METRICS.storage_access_diffs_scanned[&"base"]
