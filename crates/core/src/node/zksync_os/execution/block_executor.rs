use super::metrics;
use super::metrics::EXECUTION_METRICS;
use super::vm_wrapper::VmWrapper;
use crate::node::zksync_os::model::{BlockCommand, ReplayRecord};
use crate::node::zksync_os::storage::StateHandle;
use crate::node::zksync_os::zkos_conversions::tx_abi_encode;
use crate::node::zksync_os::BLOCK_TIME_MS;
use crate::node::{TxBatch, TxPool};
...
fn command_into_parts(
    ...
    match block_command {
        BlockCommand::Produce(ctx) => (
            ctx,
            Box::pin(futures::stream::iter(batch.txs)),
...
pub async fn execute_block(
    ...
    let (ctx, stream, seal, invalid) = command_into_parts(cmd, batch);
