...
impl InMemoryNodeInner {
    ...
    async fn run_tx<VM: VmInterface, W: WriteStorage, H: HistoryMode>(
        &mut self,
        tx: Transaction,
        tx_index: u64,
        ...
        for tx in txs {
            ...
            let result = match vm {
                AnvilVM::ZKSync(ref mut vm) => {
                    self.run_tx(tx, tx_index, block_ctx, &batch_env, vm).await
                }
    ...
    pub(super) async fn seal_block(
        ...
        let (tx_results, finished_l1_batch) = self
            .run_txs(txs, batch_env.clone(), system_env, &mut block_ctx)
            .await;
