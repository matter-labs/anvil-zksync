...
#[strum_discriminants(name(RpcCode))]
#[strum_discriminants(vis(pub))]
#[strum_discriminants(derive(AsRefStr, FromRepr))]
#[non_exhaustive]
pub enum Rpc {
    LoadStateError { inner: Box<StateLoader> } = 1u32,
    UnsupportedMethod { method_name: String } = 2u32,
    Web3Error { inner: Box<Web3> } = 3u32,
    NodeError { inner: Box<AnvilNode> } = 4u32,
    GenericError { message: String } = 0u32,
}
impl std::error::Error for Rpc {}
impl NamedError for Rpc {
    fn get_error_name(&self) -> String {
        self.as_ref().to_owned()
    }
}
impl NamedError for RpcCode {
    fn get_error_name(&self) -> String {
        self.as_ref().to_owned()
    }
}
impl From<Rpc> for crate::ZksyncError {
...
impl From<anyhow::Error> for Rpc {
    fn from(value: anyhow::Error) -> Self {
        let message = format!("{value:#?}");
        Rpc::GenericError { message }
    }
}
impl From<Rpc> for crate::packed::PackedError<crate::error::domains::ZksyncError> {
    fn from(value: Rpc) -> Self {
        crate::packed::pack(value)
    }
}
impl From<Rpc> for crate::serialized::SerializedError {
    fn from(value: Rpc) -> Self {
        let packed = crate::packed::pack(value);
        crate::serialized::serialize(packed).expect("Internal serialization error.")
    }
}
impl CustomErrorMessage for Rpc {
    fn get_message(&self) -> String {
        match self {
            Rpc::LoadStateError { inner } => {
                format!("[anvil_zksync-rpc-1] Error loading state: {inner}.")
            }
            Rpc::UnsupportedMethod { method_name } => {
                format!("[anvil_zksync-rpc-2] Unsupported method {method_name}.")
            }
            Rpc::Web3Error { inner } => {
                format!("[anvil_zksync-rpc-3] Web3 error: {inner}")
            }
            Rpc::NodeError { inner } => {
                format!("[anvil_zksync-rpc-4] Anvil-zksync node error: {inner}.")
            }
            Rpc::GenericError { message } => {
                format!("[anvil_zksync-rpc-0] Generic error: {message}")
            }
        }
    }
}
...
#[strum_discriminants(name(Web3Code))]
#[strum_discriminants(vis(pub))]
#[strum_discriminants(derive(AsRefStr, FromRepr))]
#[non_exhaustive]
pub enum Web3 {
    NoBlock = 1u32,
    PrunedBlock {
        first_retained_block: u32,
    } = 2u32,
    PrunedL1Batch {
        first_retained_batch: u32,
    } = 3u32,
    ProxyError {
        details: String,
    } = 4u32,
    ...
    InvalidFilterBlockHash = 10u32,
    MethodNotImplemented = 11u32,
    TreeApiUnavailable = 12u32,
    ServerShuttingDown = 14u32,
    GenericError {
        message: String,
    } = 0u32,
}
impl std::error::Error for Web3 {}
impl NamedError for Web3 {
    fn get_error_name(&self) -> String {
        self.as_ref().to_owned()
    }
}
impl NamedError for Web3Code {
    fn get_error_name(&self) -> String {
        self.as_ref().to_owned()
    }
}
impl From<Web3> for crate::ZksyncError {
...
impl From<anyhow::Error> for Web3 {
    fn from(value: anyhow::Error) -> Self {
        let message = format!("{value:#?}");
        Web3::GenericError { message }
    }
}
impl From<Web3> for crate::packed::PackedError<crate::error::domains::ZksyncError> {
    fn from(value: Web3) -> Self {
        crate::packed::pack(value)
    }
}
impl From<Web3> for crate::serialized::SerializedError {
    fn from(value: Web3) -> Self {
        let packed = crate::packed::pack(value);
        crate::serialized::serialize(packed).expect("Internal serialization error.")
    }
}
impl CustomErrorMessage for Web3 {
    fn get_message(&self) -> String {
        match self {
            Web3::NoBlock => {
                format!("[core-web3-1] Block with such an ID doesn't exist yet")
            }
            Web3::PrunedBlock {
                first_retained_block,
            } => {
                format ! ("[core-web3-2] Block with such an ID is pruned; the first retained block is {first_retained_block}")
            }
            Web3::PrunedL1Batch {
                first_retained_batch,
            } => {
                format ! ("[core-web3-3] L1 batch with such an ID is pruned; the first retained L1 batch is {first_retained_batch}")
            }
            Web3::ProxyError { details } => {
                format!("[core-web3-4] {details}")
            }
            Web3::SubmitTransactionError { error, data } => {
                format!("[core-web3-5] {error}")
            }
            Web3::SerializationError { details } => {
                format!("[core-web3-6] Failed to serialize transaction: {details}")
            }
            Web3::TooManyTopics => {
                format!("[core-web3-7] More than four topics in filter")
            }
            Web3::FilterNotFound => {
                format!("[core-web3-8] Filter not found")
            }
            Web3::LogsLimitExceeded {
                limit,
                from_block,
                to_block,
            } => {
                format ! ("[core-web3-9] Query returned more than {limit} results. Try with this block range [{from_block:#x}, {to_block:#x}].")
            }
            Web3::InvalidFilterBlockHash => {
                format ! ("[core-web3-10] Invalid filter: if blockHash is supplied fromBlock and toBlock must not be")
            }
            Web3::MethodNotImplemented => {
                format!("[core-web3-11] Method not implemented")
            }
            Web3::TreeApiUnavailable => {
                format!("[core-web3-12] Tree API is temporarily unavailable")
            }
            Web3::ServerShuttingDown => {
                format!("[core-web3-14] Server is shutting down")
            }
            Web3::GenericError { message } => {
                format!("[core-web3-0] Generic error: {message}")
            }
        }
    }
}
