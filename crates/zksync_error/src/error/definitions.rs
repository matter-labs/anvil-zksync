...
#[repr(u32)]
#[derive(
    AsRefStr, Clone, Debug, Eq, EnumDiscriminants, PartialEq, serde::Serialize, serde::Deserialize,
)]
...
impl CustomErrorMessage for AnvilEnvironment {
    fn get_message(&self) -> String {
        match self {
            ...
            AnvilEnvironment::Web3Error { inner } => {
    ...
    fn get_message(&self) -> String {
        match self {
            ...
            Halt::NotEnoughGasProvided {} => {
                format!(
                    "[anvil-halt-14] Bootloader did not have enough gas to start the transaction"
                )
            }
...
#[non_exhaustive]
pub enum Sequencer {
    GenericSequencerError { message: String } = 1,
    GenericError { message: String } = 0,
} // end of Sequencer
