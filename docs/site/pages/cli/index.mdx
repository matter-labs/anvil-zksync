...
{/* // TODO: @dutterbutter replace with clean installer */}

```bash // [anvil-zksync]
cargo install anvil-zksync
```
...
| ------------- | -------------------------------------------- | -------------------------- |
| **run**       | Start a brand new empty network              | [`run`](./run)             |
| **fork**      | Fork an existing chain into a local instance | [`fork`](./fork)           |
| **replay_tx** | Fork + replay a historical L2 transaction    | [`replay_tx`](./replay_tx) |
...
- **Start a local chain** → [`run`](/cli/run)
- **Fork Elastic Network chains** → [`fork`](/cli/fork)
- **Replay transaction** → [`replay_tx`](/cli/replay_tx)
